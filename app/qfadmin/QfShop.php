...
abstract class QfShop
{
    ...
    protected function initialize()
    {
        ...
        // 全后台模板可用：站点名/Logo/主题色（禁止写死）
        // 注意：think-view 无 View::share，必须用 assign（share 会 500 整站后台报错）
        View::assign('siteConf', $c);
        View::assign('siteName', $c['app_name'] ?? '资源管理后台');
        View::assign('siteLogo', !empty($c['logo']) ? $c['logo'] : (!empty($c['app_icon']) ? $c['app_icon'] : ''));
        View::assign('siteTheme', !empty($c['home_theme']) ? $c['home_theme'] : '#1e80ff');
        View::assign('siteSubname', $c['app_subname'] ?? '');
    }
    /**
     * 后台简单的身份判断
     *
