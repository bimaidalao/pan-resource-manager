...
本仓库是基于已有源码进行的二次开发，并非完全从零编写。感谢并尊重原项目作者及参与者的原创工作。

- 原始项目/源码名称：**心悦**
- 原始 README 标题：**网盘资源管理系统**
- 原始版本记录：**v3.6**（以导入时原始 README 的更新记录为准）
- 导入时源文件/目录名称：`so.laowu.life_ebykW`
- 当前二开项目名称：**Pan Resource Manager**
- 原始许可文件：[`public/install/license.txt`](public/install/license.txt)

本次二次开发主要包括 PHP 8.0/8.3 兼容、前台界面与移动端适配、资源类型筛选、搜索阶段详情预热、公开元数据与海报展示、链接检测、后台登录验证码修复、项目文档及本地开发脚本等改进。

本仓库保留对原始项目的来源说明，不主张原始源码部分的原创权。后续使用、修改或分发时，请继续尊重原作者贡献，并遵守原许可文件及其他版权声明。原始许可文件标注“版权所有，系统保留所有权利”，不应将本仓库说明理解为重新授权或放弃原作者权利。原始资料中未提供可核实的作者姓名，因此本文不擅自补写作者身份。

仓库公开后，访客可以查看、下载、克隆并在符合原许可条件的前提下部署代码，但不能直接修改本仓库或 `main` 分支。只有仓库所有者明确授权的协作者才具有写入权限；他人 Fork 后产生的修改属于其自己的仓库，不会改变本项目。
